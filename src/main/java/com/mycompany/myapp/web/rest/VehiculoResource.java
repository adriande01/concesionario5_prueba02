...
public class VehiculoResource {
    ...
    public ResponseEntity<Vehiculo> reservarVehiculo(@PathVariable Long id) {
        ...
        if (vehiculo.isPresent()) { //Si el obj existe y no hay un null en optional
            vehiculo.get().setReservado(!vehiculo.get().getReservado()); //toggle boton
            vehiculoRepository.save(vehiculo.get()); //vehiculo.get() ==> vehiculo no optional
        }
        return ResponseUtil.wrapOrNotFound(vehiculo);
    }
